...
\index{Ausfullratsel@Ausfüllrätsel}%
Das Sudoku gilt als gelöst, wenn bestimmte Regeln erfüllt sind.

Das abgebildete $4 \times 4$-Sudoku\footnote{
  Ein $4 \times 4$-Sudoku wird auch Shidoku genannt.
  Der Name Shidoku ist ein Wortspiel aus dem Wort Sudoku und dem japanischen Zahlenwort \begin{CJK}{UTF8}{min}四\end{CJK} für die Zahl 4, welches shi ausgesprochen wird. % 四
\index{Shidoku}%
\index{4@\begin{CJK}{UTF8}{min}四\end{CJK}}%
\index{shi@\begin{CJK}{UTF8}{min}四\end{CJK}}%
}
ist eine kleinere Form des üblichen $9 \times 9$-Sudokus.
...
Für jedes Feld im Sudoku gibt es eine Variable $x_{ij}$ mit den Indizes $i, j \in \{1, 2, 3, 4\}$, wobei $i$ die Zeile und $j$ die Spalte identifiziert.
In Abbildung~\ref{buchberger:sudoku-combined} ist jede Variable in ihrem Feld eingezeichnet.
Der Wert einer Variablen steht für die Ziffer, die in ihr Feld eingetragen werden soll.
...
\end{equation}
erzwungen, denn wenn einer der vier Faktoren 0 ergibt, wird die ganze Gleichung erfüllt.

Wenn man diese Idee noch für die restlichen 15 Variablen anwendet, ergeben sich die Gleichungen
\begin{equation}
\left.
\begin{aligned}
  (x_{11} - 1)\,(x_{11} - 2)\,(x_{11} - 3)\,(x_{11} - 4) &= 0,\\
...
\end{equation}
sind trivial.
Dadurch werden die vier Gleichungen aus~\eqref{buchberger:sudoku-gleichung-2}, welche $x_{13}$, $x_{31}$, $x_{34}$ bzw. $x_{42}$ betreffen, redundant.
...
Der Buchberger-Algorithmus wird gezwungen, eine Lösung zu finden, in der jede Zahl genau einmal vorkommt, sonst kann er nicht alle Gleichungen gleichzeitig erfüllen.
...
\subsubsection{Regel 5: In jedem $2 \times 2$-Block muss jede Zahl genau einmal stehen}
Diese Regel funktioniert auch wie die Regeln 3 und 4, aber anstatt Zeilen oder Spalten wird in jeder Gleichung einer der $2 \times 2$-Blöcke isoliert betrachtet:
\begin{align}
&\text{Block links oben:}
&&
\left\{\quad
\begin{aligned}
...
\begin{figure}
  \centering
  \input{papers/buchberger/images/sudokuSolved.tex}
  \caption{Das vom Buchberger-Algorithmus gelöste Sudoku.
  \label{buchberger:sudokuSolved}}
\end{figure}
Der Autor hat das Gleichungssystem mithilfe des Open-Source-Computeralgebrasystems (CAS) \emph{Maxima}~\cite{buchberger:maxima-github} gelöst.
...
Die Funktion \texttt{poly\_reduced\_\-grobner()} führt den Buchberger-Algorithmus aus und räumt auch direkt die redundanten Polynome auf.
...
\end{aligned}
\]
Die Reihenfolge der Polynome und die Anordnung der Monome scheinen willkürlich.
Sie resultieren aus der Arbeitsweise des Algorithmus und der gewählten lexikographischen Ordnung.

Indem man die Polynome sortiert, normiert und $= 0$ anfügt, erhält man das Gleichungssystem
\begin{equation}
  \left.
  \begin{aligned}
    x_{11} - 4 &= 0,&
...
\end{equation}
Daraus lassen sich die Lösungen direkt ablesen und in das Sudoku in Abbildung~\ref{buchberger:sudokuSolved} eintragen.
